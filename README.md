...
- **Install the APK**: grab `NewLeaf-1.4.0.apk` from the repo root,
...
- **CI quality gates** (all must pass on every push):
  - *Motion guard* — `scripts/verify-apk-motion.sh` inspects the built APK
    ...
    demo, leaf burst, morph, pager, haptics, back contract) ever goes missing.
  - *Perf budget (totals)* — `scripts/check-perf-budget.sh` fails if the
    payload outgrows its limits: APK ≤ 4 MiB, JS bundle ≤ 96 KiB and CSS ≤
    12 KiB (gzip'd), fonts ≤ 1.53 MiB. Every build also publishes a size
    report (now / budget / headroom / Δ vs last build) to the job summary.
  - *Perf budget (per directory)* — `scripts/check-bundle-budgets.mjs`
    attributes every module's minified size (from the stats plugin in
    `vite.config.js`) to its source area and enforces ceilings: vendor ≤ 192
    KiB, components ≤ 144 KiB, lib ≤ 48 KiB, data ≤ 20 KiB, src root ≤ 16
    KiB. Catches bloat where it *enters* — the summary names the heaviest
    modules, so a heavy dependency or asset lands with a named suspect.
    Run locally with `npm run budget:dirs`. Limits live at the top of each
    script; raise one only on purpose, with a note in the commit message.
