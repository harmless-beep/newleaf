...
test("home page loads with greeting, note of the day and tabs", async ({
  page,
}) => {
  await page.goto("/");
  ...
  await expect(
    mainNav(page).getByRole("button", { name: /My journey/ }),
  ).toBeVisible();
  await expect(
    mainNav(page).getByRole("button", { name: /Urge tools/ }),
  ).toBeVisible();
});

test("choosing a habit celebrates day one and shows in the overview", async ({
  page,
}) => {
  await page.goto("/");
  await page
    .getByRole("button", { name: /Pornography/ })
    .first()
    .click();
  await expect(page.getByText("Something to celebrate today")).toBeVisible();
  await expect(
    page.getByText(/Day one — the bravest day of all/),
  ).toBeVisible();
  await expect(page.getByText("Where you are right now")).toBeVisible();
  await expect(page.getByText(/best 1 day/)).toBeVisible();
});

test("habits and streaks persist across a reload and between tabs", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await mainNav(page)
    ...
    .click();
  await expect(page.locator(".habit-card .streak-big")).toHaveText(
    /1\s*day going strong/,
  );
...
for (const c of TIER_CASES) {
  test(`a seeded ${c.day}-day run celebrates its ${c.label} milestone`, async ({
    page,
  }) => {
    ...
    await page.addInitScript(
      ({ anchor }) => {
        ...
        localStorage.setItem(
          "nl.runs",
          JSON.stringify({ smoking: { anchor, best: 0 } }),
        );
      },
      { anchor },
    );
    ...
    await mainNav(page)
      ...
      .click();
    await expect(page.locator(".habit-card .streak-big")).toHaveText(
      new RegExp(`${c.day}\\s*days going strong`),
    );
    await expect(page.locator(".habit-card .celebration")).toContainText(
      new RegExp(`Day ${c.day}`),
    );
    await expect(page.getByText(new RegExp(c.next))).toBeVisible();
  });
}

test("a seeded month of check-ins and a long run produce a gentle monthly reflection", async ({
  page,
}) => {
  const now = new Date();
  const p = (n) => String(n).padStart(2, "0");
  ...
  await page.addInitScript(
    ...
    {
      runs: { smoking: { anchor: dateKeyAgo(40), best: 0 } },
      checkins: seeded,
    },
  );
  ...
  await expect(
    page.getByRole("heading", {
      name: new RegExp(`Your ${monthName}, gently`),
    }),
  ).toBeVisible();
  await expect(
    page.getByText(/Bright was your most common feeling/),
  ).toBeVisible();
  await expect(
    page.getByText(new RegExp(`kept every day of ${monthName} so far`)),
  ).toBeVisible();
  if (past.length >= 3) {
    await expect(
      page.getByText(/Heavy days were part of this month/),
    ).toBeVisible();
  }
});

test("morning-to-evening shifts surface as a gentle pattern note", async ({
  page,
}) => {
  ...
  await page.addInitScript(() => {
    ...
    days.forEach((k, i) => {
      checkins[k] =
        i === 4
          ? { morning: "heavy", evening: "bright" }
          : { morning: "bright", evening: "heavy" };
    });
    localStorage.setItem("nl.picks", JSON.stringify(["smoking"]));
    localStorage.setItem("nl.checkins", JSON.stringify(checkins));
  });
  ...
  await expect(
    page.getByRole("heading", { name: "Your January, gently" }),
  ).toBeVisible();
  await expect(
    page.getByText(
      /On 4 of 5 fully checked-in days you started lighter and ended restless or heavy/,
    ),
  ).toBeVisible();
  await expect(page.getByText(/worth noticing/)).toBeVisible();
});

test("the journey strip expands into a full-month calendar view and back", async ({
  page,
}) => {
  const now = new Date();
  const monthName = now.toLocaleDateString(undefined, { month: "long" });
  ...
  await expect(todayCell.locator(".month-day")).toHaveText(
    String(now.getDate()),
  );
...
test("past months can be browsed, each with its own honest reflection", async ({
  page,
}) => {
  const now = new Date();
  ...
  await expect(
    page.getByRole("heading", {
      name: new RegExp(`Your ${prevName}, looking back`),
    }),
  ).toBeVisible();
  await expect(
    page.getByText(new RegExp(`kept every day of ${prevName}\.`)),
  ).toBeVisible();
  ...
  await expect(page.getByText(`${curName} at a glance`)).toBeVisible();
  await expect(
    page.getByRole("heading", { name: new RegExp(`Your ${curName}, gently`) }),
  ).toBeVisible();
  await expect(page.getByRole("button", { name: "Next month" })).toBeDisabled();
});

test("the week strip fills in past moods and kept days from real records", async ({
  page,
}) => {
  ...
  await expect(
    strip.locator(".week-cell .week-kept").filter({ hasText: "✓" }),
  ).toHaveCount(7);
  // The cells carry each day's mood by name: oldest is Heavy, today is Bright.
  await expect(strip.locator(".week-cell").nth(0)).toHaveAttribute(
    "aria-label",
    /Heavy, all paths kept/,
  );
  await expect(strip.locator(".week-cell").nth(6)).toHaveAttribute(
    "aria-label",
    /Bright, all paths kept/,
  );
});

test("morning check-in records a mood that survives a reload and shows in the journey strip", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: /Feeling steady/ }),
  ).toBeVisible();
...
test("a skipped morning check-in is caught by a gentle optional evening check-in", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: /Morning slipped by/ }),
  ).toBeVisible();
...
test("a morning check-in can later be joined by an optional evening one", async ({
  ...
  await page.reload();
  await expect(
    page.getByRole("heading", { name: /How did today actually go\?/ }),
  ).toBeVisible();
  await expect(page.getByText(/no pressure/)).toBeVisible();
...
test("the evening check-in winds down gently and passes one small step to tomorrow", async ({
  page,
}) => {
  ...
  await page.getByRole("button", { name: /Steady/ }).click();
  await expect(
    page.getByRole("heading", { name: /Ending steady/ }),
  ).toBeVisible();
  ...
  await page
    .getByRole("textbox", { name: /one small step/ })
    .fill("a glass of water before the first coffee");
  ...
  await expect(
    page.getByText("a glass of water before the first coffee"),
  ).toBeVisible();
});

test("a slip resets the count gently and keeps the best streak", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await expect(
    page.getByRole("alertdialog", { name: "Confirm a slip" }),
  ).toBeVisible();
...
test("the journal archive groups months, searches, and looks back warmly", async ({
  page,
}) => {
      ...
      JSON.stringify([
        {
          ...
          text: "Today I felt calm for the first time in weeks.",
        },
        {
          id: "j2",
          at: "2025-08-15T08:00:00",
          text: "A hard day, but I stayed with it.",
        },
        {
          id: "j3",
          at: "2025-08-02T08:00:00",
          text: "Small steps, repeated, are unstoppable.",
        },
      ]),
    );
  });
  ...
  await page.goto("/");
  await mainNav(page)
    .getByRole("button", { name: /Urge tools/ })
    .click();
  await toolTabs(page)
    .getByRole("tab", { name: /Write it out/ })
    .click();
  ...
  await expect(
    page.getByText("Today I felt calm for the first time in weeks."),
  ).toBeVisible();
  await expect(
    page.getByText("A hard day, but I stayed with it."),
  ).not.toBeVisible();
  await expect(
    page.getByRole("heading", { name: "August 2025" }),
  ).not.toBeVisible();
...
test("a month exports as a keepsake PDF with calendar, reflection and journal", async ({
  page,
}) => {
  await page.clock.install({ time: new Date(2026, 0, 20, 9, 0) });
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "nl.runs",
      JSON.stringify({ smoking: { anchor: "2026-01-01", best: 0 } }),
    );
    localStorage.setItem(
      "nl.checkins",
      JSON.stringify({
        "2026-01-20": { morning: "bright", evening: "heavy" },
        "2026-01-19": "steady",
      }),
    );
    localStorage.setItem(
      ...
      JSON.stringify([
        {
          id: "k1",
          at: "2026-01-15T08:00:00",
          text: "A calm morning, a hard evening — I stayed anyway.",
        },
        {
          id: "k2",
          at: "2026-01-03T08:00:00",
  ...
  await expect(
    sheet.getByText(/kept every day of January so far/),
  ).toBeVisible();
  await expect(
    sheet.getByText("A calm morning, a hard evening — I stayed anyway."),
  ).toBeVisible();
...
test("a streak can be kept or shared as a quiet PNG card, drawn on-device", async ({
  page,
}) => {
  await page.goto("/");
  await page
  ...
  await expect(
    card.getByRole("heading", { name: /A quiet card for Pornography/ }),
  ).toBeVisible();
  ...
  const [download] = await Promise.all([
    ...
    card.getByRole("button", { name: "Download PNG" }).click(),
  ]);
  expect(download.suggestedFilename()).toMatch(
    /new-leaf-pornography-day-1\.png/,
  );
...
test("any month can be drawn into a quiet celebration card", async ({
  page,
}) => {
  await page.clock.install({ time: new Date(2026, 0, 20, 9, 0) });
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "nl.checkins",
      JSON.stringify({ "2026-01-20": "steady", "2026-01-19": "bright" }),
    );
  });
  await page.goto("/");
  await mainNav(page)
  ...
  const card = page.locator(".celebrate");
  await expect(
    card.getByRole("heading", { name: /A quiet card for your month/ }),
  ).toBeVisible();
  await expect(card.locator("canvas")).toHaveAttribute("width", "1080");
...
test("closed months are preserved automatically as keepsakes and listed on My journey", async ({
  page,
}) => {
      ...
      JSON.stringify([
        {
          ...
          text: "Two weeks in — quieter mind.",
        },
      ]),
    );
  });
  ...
  await expect(
    shelf.getByText(
      /Bright was your most common feeling — on 2 of 3 days you checked in\./,
    ),
  ).toBeVisible();
  ...
  const saved = await page.evaluate(() =>
    JSON.parse(localStorage.getItem("nl.keepsakes")),
  );
  ...
  await expect(
    page.getByRole("heading", { name: "Your January, looking back" }),
  ).toBeVisible();
  await expect(page.getByText("kept every day of January.")).toBeVisible();
});

test("a keepsake month stays true even after a later slip would silence it", async ({
  page,
}) => {
  ...
  await page.addInitScript(() => {
    ...
    localStorage.setItem(
      "nl.runs",
      JSON.stringify({ smoking: { anchor: "2026-02-05", best: 31 } }),
    );
    localStorage.setItem(
      "nl.checkins",
      JSON.stringify({ "2026-01-05": "bright" }),
    );
    const days = [];
    for (let d = 1; d <= 31; d++)
      days.push({ d, moodId: d === 5 ? "bright" : null, kept: 1, tracked: 1 });
    localStorage.setItem(
      "nl.keepsakes",
      JSON.stringify({
        "2026-01": {
          ...
          closing:
            "A month with heavy days in it is still a month you lived through.",
  ...
  await expect(page.getByText("kept every day of January.")).toBeVisible();
  await expect(
    page.locator(".month-cell .week-kept").filter({ hasText: "✓" }),
  ).toHaveCount(31);
  ...
  await expect(sheet.locator(".ks-mark").filter({ hasText: "✓" })).toHaveCount(
    31,
  );
  await expect(sheet.locator(".ks-mark").filter({ hasText: "—" })).toHaveCount(
    0,
...
test("urge tools: breathing, grounding, ride the wave and journal all work", async ({
  page,
}) => {
  await page.goto("/");
  await mainNav(page)
  ...
  await expect(
    page.getByRole("button", { name: "Start breathing" }),
  ).toBeVisible();

  // Grounding: walk all five senses to completion
  await toolTabs(page)
    .getByRole("tab", { name: /Ground/ })
    .click();
  for (let i = 0; i < 4; i++) {
  ...
  await toolTabs(page)
    .getByRole("tab", { name: /Ride it out/ })
    .click();
  ...
  await toolTabs(page)
    ...
    .click();
  await page
    .getByPlaceholder(/What’s happening right now\?/)
    .fill("Today was hard, but I stayed with it.");
  await page.getByRole("button", { name: "Let it out" }).click();
  await expect(
    page.getByText("Today was hard, but I stayed with it."),
  ).toBeVisible();
  await page.getByRole("button", { name: "Delete entry" }).click();
  await expect(page.getByText("Nothing here yet.")).toBeVisible();
});

test("a first-time visitor is welcomed warmly, and “Begin gently” starts the day", async ({
  page,
}) => {
  ...
  await expect(
    page.getByRole("heading", { name: "Welcome to New Leaf" }),
  ).toBeVisible();
  ...
  await demo.click();
  await expect(
    dialog.getByRole("button", { name: /^Breathe (in|out) \d+$/ }),
  ).toBeVisible();
  await expect(
    dialog.getByText("Follow the circle. That’s all it asks."),
  ).toBeVisible();
...
test("choosing a habit sets off a gentle one-time leaf burst for day one", async ({
  page,
}) => {
  await page.goto("/");
  await page
...
test("reaching a milestone day bursts once on open, and not again on a reload", async ({
  page,
}) => {
...
test.describe("phone layout", () => {
  ...
  test("tabs live in a fixed bottom bar for one-hand use", async ({ page }) => {
    ...
    await expect(
      page.getByText("A path begins with a single step — choose yours"),
    ).toBeVisible();
    ...
    await expect(
      page.getByRole("heading", { name: "Urge tools" }),
    ).toBeVisible();
  });
});

test("the native-splash hold keeps entrance animations until the release moment", async ({
  page,
}) => {
...
test("the entrance-hold self-heals if the native release call is lost", async ({
  page,
}) => {
  // The lost-release race: an early tap-to-skip (or the safety net) can call
  // the release before the page's JS has evaluated, so the wrapper's call
  // becomes a no-op and the hold would strand every animation. Inside a
  // mocked wrapper the watchdog must drop the hold on its own. On the plain
  // web (no AndroidNative) no hold is ever added, so this only works because
  // the init script installs the bridge before the app bundle evaluates.
  await page.addInitScript(() => {
