...
check "$css" '@keyframes detailSheetIn'  'habit detail shared-element morph (open)'
check "$css" '@keyframes detailSheetOut' 'habit detail shared-element morph (close)'
check "$css" 'detail-ladder'            'habit detail milestone ladder'
check "$js"  'detail-strip'             'habit detail 14-day strip'
check "$js"  '--ms'                     'morph scale custom property (FLIP)'
check "$js"  'pager-dragging'           'month pager drag controller'
check "$css" 'touch-action:pan-y'       'pager axis lock (vertical scroll stays native)'
check "$css" 'overscroll-behavior:contain' 'scroll chaining containment (no edge glow)'
check "$js"  '__nlAndroidBack'          'hardware-back overlay contract (page side)'
check_dex '__nlAndroidBack'              'hardware-back overlay contract (native side)'
